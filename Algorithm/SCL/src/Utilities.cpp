...
// SCL f(y1, y2) = sgn(y1)*sgn(y2)*min(abs(y1), abs(y2))
...
double PM() {
    ...
    if () return 0; 
        ...
        // if "estimate_u" is frozen bit and incorrect value 
        if () { 
            return numeric_limits<double>::max(); 
        }
        // if "estimate_u" is information or frozen bit and "estimate_u == delta(LLR_u)"
        else if () {
            return  
        }
        // if "estimate_u" is information or frozen bit and "estimate_u != delta(LLR_u)"
